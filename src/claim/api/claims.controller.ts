...
export class ClaimController {
    ...
    async register(
        ...
        try{
            const result: Result<AppNotification, RegisterClaimResponse> = await this.claimApplicationService.register(registeredClaimRequest);
            if (result.isSuccess())
            {
    ...
    @Get('/:id')
    async getById(@Param('id') claimId: number, @Res({passthrough: true}) response) : Promise<object>
    {
        try 
        {
            const customers = await this.queryBus.execute(new GetClaimByIdQuery(claimId));
            return ApiController.ok(response, customers);
        }
        catch(error)
        {
